...
from ultralytics.data.utils import check_cls_dataset, check_det_dataset,check_det_dual_dataset
...
class BaseDualTrainer:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        """print("i am here inside base trainer ")"""
        ...
        with torch_distributed_zero_first(LOCAL_RANK):  # avoid auto-downloading dataset multiple times
            self.trainset, self.testset = self.get_dataset() # i reached here yesterday
        self.ema = None
        ...
        while True:
            ...
            for i, batch in pbar:
                ...
                if RANK in {-1, 0}:
                    ...
                    pbar.set_description(
                        ("%11s" * 2 + "%11.4g" * loss_length + "%11d" * 3) % (
                            f"{epoch + 1}/{self.epochs}",
                            f"{self._get_memory():.3g}G",
                            *(self.tloss if loss_length > 1 else torch.unsqueeze(self.tloss, 0)),
                            batch["cls"].shape[0],
                            batch["img"].shape[-1],
                            batch["ir_img"].shape[-1]
                        )
                    )
            ...
            if RANK in {-1, 0}:
                ...
                if self.args.val or final_epoch or self.stopper.possible_stop or self.stop:
                    """*************************************************************************************************************"""
                    self.metrics, self.fitness = self.validate() # start working on here for valiadation model
    ...
    def get_dataset(self):
        ...
        try:
            if self.args.task == "classify":
                ...
                data = check_det_dual_dataset(self.args.data)
