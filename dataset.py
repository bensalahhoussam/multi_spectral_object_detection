...
from .augment import (
    ...
    Format,FormatDual,
    Instances,
    LetterBox,LetterBoxDual,
    RandomLoadText,
    classify_augmentations,
    classify_transforms,
    v8_transforms,
)
from .base import BaseDataset,DualBaseDataset
from .utils import (
    HELP_URL,
    LOGGER,
    get_hash,
    ...
    save_dataset_cache_file,verify_image,verify_image_label)

# Ultralytics dataset *.cache version, >= 1.0.0 for YOLOv8
DATASET_CACHE_VERSION = "1.0.3"

class YOLODualDataset(DualBaseDataset):
    ...
    def build_transforms(self, hyp=None):
        ...
        if self.augment:
            ...
            transforms = Compose([LetterBoxDual(new_shape=(self.imgsz, self.imgsz), scaleup=False)])
        transforms.append(
            FormatDual(
                bbox_format="xywh",
                normalize=True,
...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        print(path)# number missing, found, empty, corrupt, messages
        desc = f"{self.prefix}Scanning {path.parent / path.stem}..."
        total = len(self.im_files)
        nkpt, ndim = self.data.get("kpt_shape", (0, 0))
        if self.use_keypoints and (nkpt <= 0 or ndim not in {2, 3}):
